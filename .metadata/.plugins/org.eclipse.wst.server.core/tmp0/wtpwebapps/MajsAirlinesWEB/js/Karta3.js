$(document).ready(function() {
	...
	if(window.location.search.slice(1).split('&').length>1){
		idP = window.location.search.slice(1).split('&')[1].split('=')[1];
	}
	console.log("id"+id);
	console.log("idP: "+idP);
	var params1={
	...
	getCena(params1);
	getSedistaPolazni(paramsPolazni);
	getSedistaPovratni(paramsPovratni);
...
function getSedistaPolazni(paramsPolazni){
	//var paramsPolazni = {"action": "polazniSedista","letIdPol": id};
...
function getSedistaPovratni(paramsPovratni){
	//var paramsPovratni = {"action": "povratniSedista","letIdPol": idP};
